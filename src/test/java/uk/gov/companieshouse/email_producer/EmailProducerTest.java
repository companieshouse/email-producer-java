...
public class EmailProducerTest {
    ...
    public void sendEmail__serializationError() throws SerializationException, JsonProcessingException {
    ...
    @Test(expected=EmailSendingException.class)
    public void sendEmail__sendingError() throws SerializationException, ExecutionException,
            InterruptedException, JsonProcessingException {
        
        byte[] testBytes = new byte[]{ 0x1, 0x2, 0x3 };
        when(emailFactory.buildEmail(emailData, "test-app", "test-message")).thenReturn(mockEmail);
        when(emailSerializer.toBinary(mockEmail)).thenReturn(testBytes);
        doThrow(new ExecutionException("TEST ERROR"){}).when(chKafkaProducer).send(any());
    ...
    @Test(expected=EmailSendingException.class)
    public void sendEmail__interruption() throws SerializationException, ExecutionException,
            InterruptedException, JsonProcessingException {
        
        byte[] testBytes = new byte[]{ 0x1, 0x2, 0x3 };
        when(emailFactory.buildEmail(emailData, "test-app", "test-message")).thenReturn(mockEmail);
        when(emailSerializer.toBinary(mockEmail)).thenReturn(testBytes);
        doThrow(new InterruptedException("TEST ERROR")).when(chKafkaProducer).send(any());
